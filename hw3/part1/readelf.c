...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
	...
	warn (_("Size of section %u is larger than the entire file!\n"), i);
      ...
      printf ("  [%2u] ", i);
      if (do_section_details){
		if(section->sh_type != SHT_NULL){ // RB
			printf ("%s_<3KIREL\n      ", printable_section_name (filedata, section));
		} else {
			printf ("%s\n      ", printable_section_name (filedata, section));
		}
		...
		if(section->sh_type != SHT_NULL){ // RB
			char* kirel = malloc(strlen(SECTION_NAME_PRINT (section)) + strlen("_<3KIREL") + 1);
			sprintf(kirel, "%s%s", SECTION_NAME_PRINT (section), "_<3KIREL");
			print_symbol (-17, kirel); // RB
			free(kirel);
		} else {
			print_symbol (-17, SECTION_NAME_PRINT (section)); // RB
		}
	  }
...
static int global_counter = 0; // RB
...
get_symbol_binding (Filedata * filedata, unsigned int binding)
{
  ...
  switch (binding)
    ...
    case STB_GLOBAL:	global_counter++; return "GLOBAL";
